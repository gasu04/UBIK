...
Examples:
  ...
  %(prog)s local ./therapy --mcp-host 100.103.242.91
...
async def create_mcp_client(args: argparse.Namespace):
    """
    Create and connect an MCP client for Hippocampal Node storage.

    Resolution order for host/port:
      1. CLI flags (--mcp-host, --mcp-port)
      2. Environment variables (HIPPOCAMPAL_HOST, HIPPOCAMPAL_PORT/HIPPOCAMPAL_MCP_PORT)
      3. HippocampalClient defaults

    Returns:
        Connected HippocampalClient, or None if dry-run or connection fails
    """
    if getattr(args, 'dry_run', False):
        logger.info("Dry run mode - skipping MCP connection")
        return None
    ...
    try:
        somatic_dir = str(Path.home() / "ubik" / "somatic")
        if somatic_dir not in sys.path:
            sys.path.insert(0, somatic_dir)

        from mcp_client import HippocampalClient

        client = HippocampalClient(**kwargs)

        resolved_host = getattr(client, 'host', host or '?')
        resolved_port = getattr(client, 'port', port or '?')
        logger.info(
            f"MCP client created for {resolved_host}:{resolved_port} "
            f"(connection handled by pipeline)"
        )
        return client

    except ImportError:
        logger.warning(
            "mcp_client package not found at ~/ubik/somatic/mcp_client/. "
            "Running without storage (dry run mode)."
        )
        return None
    except Exception as e:
        display_host = host or os.environ.get("HIPPOCAMPAL_HOST", "not set")
        display_port = port or os.environ.get("HIPPOCAMPAL_PORT", "8080")
        logger.warning(
            f"Could not connect to Hippocampal Node at {display_host}:{display_port}: {e}\n"
            "Running without storage (dry run mode)."
        )
        return None
...
def _purge_source_files(filenames: List[str]) -> Dict[str, int]:
    ...
    _load_maestro_env()
    host = os.environ.get("HIPPOCAMPAL_TAILSCALE_IP", "100.103.242.91")
