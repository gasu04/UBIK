...
## Session: [2026-07-29 19:47] — [Node: Somatic]
**Goal:** Push HARDENING_PLAN Layer B (Windows Scheduled Tasks) toward completion while remote (no physical/RDP access to the Somatic Windows host), and separately implement Layer D (SSH/probe resilience).
**Completed:**
- Confirmed the last SSH disconnect ("Connection to 100.75.228.46 closed.") was benign, not the `p9io` VM-teardown bug: `journalctl --list-boots` shows the current WSL boot has been continuously up since 2026-07-23 19:29:28 PDT, unchanged through the reconnect.
- Diagnosed and fixed "vLLM not starting": WSL's own Tailscale identity was `NeedsLogin` (no IPv4 assigned) — separate from the Windows host's own Tailscale identity (runs under `Adrian\adria`, server mode). Since maestro's health probe targets the Somatic Tailscale IP, an unauthenticated WSL Tailscale meant probes could never reach the box regardless of whether vLLM itself was healthy. Fixed via `sudo tailscale up` + browser re-auth ("Success.").
- Implemented **HARDENING_PLAN Layer D** (SSH + vLLM probe resilience, CLAUDE.md §2.3): `RemoteExecutor.run` and `VllmService.probe` now gated by the canonical Probe-Latch `CircuitBreaker`; `base._wait_for_healthy`'s fixed 3s poll cadence replaced with `poll_interval + up to 20% jitter` (deliberately not exponential growth — an early version grew the interval and blew through an existing test's timing budget, and growth fights the loop's actual job of detecting "just became healthy" fast; the breaker owns the "stop hammering a dead node" concern). New `maestro/_canonical_resilience.py` loads `somatic/mcp_client/resilience.py` by file path rather than a normal package import, because `somatic/mcp_client/__init__.py` transitively imports `hippocampal_client.py`, which calls `logging.basicConfig()` at import time — a real pre-existing bug that leaked stray log handlers into maestro's root logger and broke `test_logger.py` (2 failures) until routed around. Added 10 new tests (breaker-opens-after-N-failures, reject-fast-when-open, HALF_OPEN admits exactly one probe, poll-delay bounds). Verified 649/649 passing (639 baseline + 10 new) in a throwaway venv built from `maestro/requirements.txt`. Committed `efa6373` and pushed to `origin/master`.
- Layer B (Windows Scheduled Tasks) worked entirely remotely via SSH interop from this WSL session, with no physical/RDP access to the Somatic Windows host:
  - Staged `somatic/windows/*.ps1`/`*.xml` to `C:\UBIK\somatic\windows\` (non-admin step, done directly from WSL via `cp` into `/mnt/c/`).
  - Diagnosed why `gasu` (the SSH/WSL account) isn't Windows-admin and can't self-elevate: confirmed via `wevtutil qe Security` (admin-only probe) returning "Access is denied" that WSL interop carries a UAC-filtered, non-elevated token — even a network-logon SSH path doesn't bypass UAC here, unlike the theoretical PsExec/WinRM case.
  - User got `gasu` promoted to local Administrator via someone else approving a UAC prompt in an elevated session (`net localgroup Administrators gasu /add` — confirmed via `net localgroup Administrators` listing `Administrator`, `adria`, `gasu`).
  - Re-tested elevation after the promotion: still non-elevated (`wevtutil` still denied) — confirmed admin group membership does not retroactively elevate an already-running WSL interop session. The scheduled-task registration still requires a genuine interactive elevated session (physical console or RDP), not achievable over SSH.
  - Checked RDP availability as a possible alternative remote path: **disabled** (`fDenyTSConnections=0x1`, firewall rule "Remote Desktop" Enabled=No). Windows 10 Pro (can host RDP once enabled), but enabling it itself needs the same elevated session Layer B is blocked on — not a shortcut this round.
  - **New breakage found and diagnosed (not yet fixed):** after the admin promotion, `ssh windows-server` from Hippocampal started failing with "Permission denied (publickey)". Root cause: `sshd_config`'s `Match Group administrators` block redirects `AuthorizedKeysFile` to `C:\ProgramData\ssh\administrators_authorized_keys` for any account in `Administrators` — now including `gasu`. His working key (fingerprint `SHA256:MGfeJUuYFDVUU/rK5vIDSIb4VxXnll3dOIVGdg/4nm8`, labelled `dad@mac`) is only in his per-user `authorized_keys` (confirmed present there, unchanged), not in the admin-only file (confirmed unreadable as `gasu` — expected, tight ACL). Fix identified but not yet applied (needs another elevated session): `Add-Content` the `dad@mac` key line into `administrators_authorized_keys` (append, never overwrite — `adria`'s key is presumably already in there), then re-tighten its ACL with `icacls.exe ... /inheritance:r /grant "Administrators:F" /grant "SYSTEM:F"` per Win32-OpenSSH's strict-permissions requirement.
  - Incidental finding, not fixed: a third line in `gasu`'s per-user `authorized_keys` looks corrupted/truncated (placeholder `AAAA...` under a duplicate `dad@mac` label) — dead, non-functional, low priority cleanup.
  - Also incidental: `gasu`'s Windows password shows `expires 1/31/2026` (already past) — Windows will likely force a password change at his next interactive/console login; expected, not a bug.
**State left in:**
- Layer D: done, tested, committed, and pushed (`efa6373`) — no longer blocked on anything.
- vLLM/Tailscale: WSL-side Tailscale re-authenticated; IP should be re-verified against `SOMATIC_TAILSCALE_IP` in Hippocampal's `maestro/.env` (not yet confirmed to match).
- Layer B: `C:\UBIK\somatic\windows\*` staged on the Windows host. Scheduled Tasks still NOT registered — genuinely needs one more elevated interactive session on that host (console or RDP; RDP is currently off). `gasu` is now a local Administrator, which removes the need for cross-account (`adria`) involvement in that session once it happens, but does NOT make the session reachable via SSH/WSL interop.
- SSH access for `gasu` from Hippocampal (`ssh windows-server`) is currently **broken** (publickey denied) due to the `Match Group administrators` file-redirect described above. This blocks the normal remote workflow (including any future WSL-interop-based diagnostics as `gasu`) until fixed.
**Files changed:**
- maestro/_canonical_resilience.py: new — file-path import shim for the canonical resilience module (bypasses a logging.basicConfig() side effect in somatic/mcp_client/__init__.py's import chain)
- maestro/remote.py: RemoteExecutor.run gated by CircuitBreaker; version 0.1.0 -> 0.2.0
- maestro/services/vllm_service.py: probe() gated by CircuitBreaker; version 0.8.0 -> 0.9.0
- maestro/services/base.py: _wait_for_healthy poll delay -> poll_interval + jitter (canonical calculate_backoff_delay), no growth
- maestro/config.py: SomaticConfig +circuit_breaker_failure_threshold, +circuit_breaker_recovery_timeout_s
- maestro/services/__init__.py: _register_all wires real circuit-breaker config into RemoteExecutor + VllmService
- maestro/tests/test_remote.py, test_service_probes.py, test_liveness.py: +10 tests for Layer D
- ubik_sessions.md: this entry
**Next session should:**
1. Get one more elevated Windows session (console or RDP once enabled) to: (a) fix `gasu`'s broken SSH access by appending his key to `administrators_authorized_keys` + re-tightening its ACL (commands identified above), and (b) register the two Scheduled Tasks via `schtasks /Create ... /XML ... /F` (files already staged at `C:\UBIK\somatic\windows\`). Both can be done in the same elevated window.
2. Re-verify `tailscale ip -4` on Somatic matches `SOMATIC_TAILSCALE_IP` in Hippocampal's `maestro/.env`; update if it drifted after the re-auth.
