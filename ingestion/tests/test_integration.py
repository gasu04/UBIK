...
# =============================================================================
# LocalMemoryWriter + INTELLECTUAL type tests
# =============================================================================

class TestIntellectualMemoryType:
    """Unit tests for the INTELLECTUAL memory type added during migration."""

    def test_intellectual_in_enum(self):
        assert MemoryType.INTELLECTUAL.value == "intellectual"

    def test_all_types_present(self):
        names = {m.name for m in MemoryType}
        assert {"EPISODIC", "SEMANTIC", "INTELLECTUAL", "SKIP"} == names

    def test_intellectual_candidate_construction(self):
        from datetime import datetime
        candidate = MemoryCandidate(
            content="Borges writes that the map is not the territory...",
            memory_type=MemoryType.INTELLECTUAL,
            confidence=0.85,
            category="literature",
            themes=["philosophy", "maps", "reality"],
            emotional_valence="curious",
            source_file="borges_quotes.txt",
            source_chunk_index=0,
            importance=0.75,
        )
        assert candidate.memory_type == MemoryType.INTELLECTUAL

    def test_storage_stats_has_intellectual_field(self):
        from ingest.pipeline import StorageStats
        s = StorageStats()
        assert hasattr(s, "intellectual_stored")
        assert s.intellectual_stored == 0
        s.intellectual_stored += 1
        assert s.intellectual_stored == 1


class TestLocalMemoryWriterImport:
    """Smoke-tests for LocalMemoryWriter module-level imports."""

    def test_import(self):
        from ingest.mcp_writer import LocalMemoryWriter, FrozenVoiceError
        assert LocalMemoryWriter is not None
        assert FrozenVoiceError is not None

    def test_interface_methods(self):
        from ingest.mcp_writer import LocalMemoryWriter
        for method in (
            "connect", "close", "store_episodic", "store_semantic",
            "store_intellectual", "update_identity_graph", "get_stats",
        ):
            assert hasattr(LocalMemoryWriter, method), f"Missing method: {method}"

    def test_context_manager_protocol(self):
        from ingest.mcp_writer import LocalMemoryWriter
        assert hasattr(LocalMemoryWriter, "__aenter__")
        assert hasattr(LocalMemoryWriter, "__aexit__")

    def test_pipeline_routes_intellectual(self):
        """_store_intellectual exists and pipeline branches on INTELLECTUAL."""
        from ingest.pipeline import IngestPipeline
        import inspect
        src = inspect.getsource(IngestPipeline._store_memories)
        assert "INTELLECTUAL" in src
        assert "_store_intellectual" in src
