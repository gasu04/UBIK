    ...
    async def test():
        async with HippocampalClient() as client:
            if client.is_connected:
                print('  \033[0;32m✓\033[0m Connected to local MCP server')
            else:
...
echo
echo "Next steps:"
echo "  1. Prepare your source materials in /Volumes/990PRO 4T/UBIK/data/source_materials/"
echo "  2. Run a dry-run test:"
echo "     python -m ingest.cli local '/Volumes/990PRO 4T/UBIK/data/source_materials/' --dry-run"
echo "  3. Use interactive mode for review:"
echo "     python interactive_ingest.py '/Volumes/990PRO 4T/UBIK/data/source_materials/'"
echo "  4. Ingest with local MCP server (running on this node):"
echo "     python -m ingest.cli local '/Volumes/990PRO 4T/UBIK/data/source_materials/' --mcp-host localhost"
echo
