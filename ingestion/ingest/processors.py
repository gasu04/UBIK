...
class AudioProcessor(BaseProcessor):
    """
    Processor for audio files using WhisperX (Somatic) or local Whisper.

    Transcription strategy:
      1. If SOMATIC_HOST and SOMATIC_WHISPER_PORT are set, delegate to the
         WhisperX FastAPI server on the Somatic Node (GPU-accelerated).
      2. Otherwise, fall back to local openai-whisper on CPU.

    Supported: .mp3, .wav, .m4a, .ogg, .flac

    Features:
        - Somatic GPU delegation with automatic local fallback
        - Configurable model size for CPU fallback (tiny/base/small)
        - Language detection
    ...
    def __init__(self, config: Optional[ProcessorConfig] = None):
        ...
        self._somatic_host = os.getenv("SOMATIC_HOST")
        self._somatic_port = int(os.getenv("SOMATIC_WHISPER_PORT", "9100"))
    ...
    async def _transcribe_remote(self, file_path: Path) -> Optional[Dict[str, Any]]:
        """Try to transcribe via the Somatic WhisperX server.

        Returns the result dict on success, or None if Somatic is unavailable.
        """
        if not self._somatic_host:
            return None

        import httpx

        url = f"http://{self._somatic_host}:{self._somatic_port}/transcribe"
        try:
            ...
            if resp.status_code == 200:
                logger.info(f"Remote transcription succeeded via {url}")
                return resp.json()
            logger.warning(
                f"Remote transcription returned HTTP {resp.status_code}, "
                "falling back to local Whisper"
            )
            return None
        except Exception as exc:
            logger.warning(
                f"Remote transcription failed ({exc}), falling back to local Whisper"
            )
            return None
    ...
    async def process(
        ...
        Attempts Somatic GPU transcription first; falls back to local Whisper.
        Runs blocking work in a thread pool to avoid blocking the async loop.
        """
        start_time = time.time()

        # Try GPU path on Somatic Node first
        result = await self._transcribe_remote(file_path)

        if result is None:
            # Local CPU fallback
            model = await self._get_model()
            result = await self._run_in_executor(self._transcribe, model, file_path)

        model_used = result.get("model_type") or result.get("model_used") or self.config.whisper_model
