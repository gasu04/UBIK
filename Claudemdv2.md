...
The architectural standards in Sections 1–10 define the mandatory baseline for this project. The behavioral guidelines in Section 11 (Karpathy Principles) govern *how* Claude approaches work within that baseline.

**When they conflict, Sections 1–10 take precedence.** Karpathy's "Simplicity First" and "Surgical Changes" principles apply to code not already governed by an explicit UBIK mandate. For example: if UBIK mandates a circuit breaker for all external service calls, that mandate stands regardless of whether it was explicitly requested in the task. If UBIK mandates module docstrings, those must be added even when editing an existing file that is missing them.
...
11. [Behavioral Guidelines (Karpathy Principles)](#11-behavioral-guidelines-karpathy-principles)
12. [Claude Code Session Management](#12-claude-code-session-management)
...
## 11. Behavioral Guidelines (Karpathy Principles)

Derived from Andrej Karpathy's observations on LLM coding pitfalls. These principles govern Claude's *approach and process* within the constraints set by Sections 1–10. See [Document Precedence](#document-precedence) for conflict resolution.
...
- No features beyond what was asked — *unless mandated by Sections 1–10.*
...
- Don't "improve" adjacent code, comments, or formatting — *unless a UBIK standard explicitly requires it (e.g., Section 5.1 module docstrings must be added if missing).*
...
The test: Every changed line should trace directly to the user's request *or to a mandatory UBIK standard.*
...
### 12.1 Model Usage Guidelines

`CLAUDE.md` is re-read from disk at the start of every session. Use this section to guide model selection across all sessions on this project.

| Task type | Model |
|-----------|-------|
| Bug fixes, unit tests, documentation | Sonnet 4.6 |
| Refactoring core architecture, resolving complex race conditions | Opus 4.6 |

Override model for the current session: `/model <alias|name>` (e.g., `/model opus`, `/model sonnet`)

### 12.2 Configuration Priority

When multiple model settings exist, Claude Code resolves them in this order (highest to lowest):

1. **Session command** — `/model <alias|name>` (overrides everything for the current session)
2. **CLI flag** — `claude --model <alias|name>`
3. **Environment variable** — `export ANTHROPIC_MODEL=<alias|name>`
4. **Settings file** — `~/.claude/settings.json` → `"model": "sonnet"` (lowest priority)

### 12.3 Context Window Management

Run `/compact` every 20–30 turns to keep the context window lean and costs low.

**What `/compact` affects:** The conversational history of the current session is summarized and compressed.

**What `/compact` does NOT affect:**
- `CLAUDE.md` — always re-read fresh from disk at session start
- Auto-memory files at `~/.claude/projects/*/memory/` — file-based, persist independently
- Any file edits already written to disk
